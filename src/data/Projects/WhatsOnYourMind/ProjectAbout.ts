import PlatformsAbout from './PlatformsAbout.svelte';
import DanielAbout from './DanielAbout.svelte';
import H4wneeAbout from './H4wneeAbout.svelte';

export const whatsOnYourMindPlatformsAbout = {
	title: 'platforms limitations',
	subtitle: '',
	text: PlatformsAbout
};

export const whatsOnYourMindPlatformsImages = ['/media/whats-on-your-mind/about-1.webp'];

export const whatsOnYourMindDanielAbout = {
	title: 'DANIEL VASCONCELOS',
	subtitle: '',
	text: DanielAbout
};

export const whatsOnYourMindDanielImages = [
	'/media/whats-on-your-mind/about-21.webp',
	'/media/whats-on-your-mind/about-22.webp'
];

export const whatsOnYourMindH4wneeAbout = {
	title: 'H4WNEE',
	subtitle: '',
	text: H4wneeAbout
};

export const whatsOnYourMindH4wneeImages = [
	'/media/whats-on-your-mind/about-31.webp',
	'/media/whats-on-your-mind/about-32.webp'
];

