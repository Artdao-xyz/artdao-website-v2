...
<button
	...
	class="relative flex flex-row {!isSelected
		? 'gray-gradient'
		: 'dark-gradient'} items-center justify-center rounded-40 py-[15px] px-[17px] w-full h-[5.4375rem] {!isSelected
		? 'unselected'
		: 'selected'}"
>
	<p
...
{#if isSelected}
	<div class="flex flex-col justify-center items-center gap-[5px] w-full">
		{#each mapLocation.events as event}
			<button
				on:click={() => handleOnClick(event)}
				class="flex flex-rowitems-center justify-center rounded-40 w-full h-[47px] gray-gradient"
			>
				<p
